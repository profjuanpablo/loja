<?php
	if(!empty($_GET['id'])){
		...
		$result_sit_carrousel = "SELECT * FROM situacoes_carrouses WHERE id = '$id' LIMIT 1";
		$resultado_sit_carrousel = mysqli_query($conn, $result_sit_carrousel);
		$row_sit_carrousel = mysqli_fetch_assoc($resultado_sit_carrousel);	
	}
?>
<div class="container theme-showcase" role="main">
	<div class="page-header">
        <h1>Editar Situação para Carrousel</h1>
	</div>
	<div class="row">
	...
	<form name="adm_cad_sit_carrousel" class="form-horizontal" method="POST" action="administrativo/processa/adm_proc_edita_sit_carrousel.php" enctype="multipart/form-data">
		<div class="form-group">
			<label class="col-sm-2 control-label">Nome</label>
			<div class="col-sm-10">
				<input type="text" name="nome" class="form-control" id="inputEmail3" placeholder="Nome" required
				<?php
					if(!empty($row_sit_carrousel['nome'])){
						echo "value='".$row_sit_carrousel['nome']."'";
					}
			...
			<div class="col-sm-10">
				<select class="form-control" name="cor">
					...
					<option value='default'
						...
						if(!empty($row_sit_carrousel['cor'])){
							if($row_sit_carrousel['cor'] == "default"){
								echo 'selected';
							}
						} 
						?>
					...
					<option value='primary'
						...
						if(!empty($row_sit_carrousel['cor'])){
							if($row_sit_carrousel['cor'] == "primary"){
								echo 'selected';
							}
						} ?>
					>Azul escuro</option>
					<option value='success'
						...
						if(!empty($row_sit_carrousel['cor'])){
							if($row_sit_carrousel['cor'] == "success"){
								echo 'selected';
							}
						} ?>
					>Verde</option>
					...
					<option value='warning'
						...
						if(!empty($row_sit_carrousel['cor'])){
							if($row_sit_carrousel['cor'] == "warning"){
								echo 'selected';
							}
						} ?>
					>Laranjado</option>
					<option value='danger'
						...
						if(!empty($row_sit_carrousel['cor'])){
							if($row_sit_carrousel['cor'] == "danger"){
								echo 'selected';
							}
						} ?>
					>Vermelho</option>
		...
		<input type="hidden" name="id" 
			<?php
				...
				if(!empty($row_sit_carrousel['id'])){
					echo "value='".$row_sit_carrousel['id']."'";
				}
			?>>
		<div class="form-group">
			<div class="col-sm-offset-2 col-sm-10">
				<input type="submit" class="btn btn-warning" value="Editar" onclick="return val_adm_cad_sit_carrousel()">
			</div>
		</div>
	</form>
</div>