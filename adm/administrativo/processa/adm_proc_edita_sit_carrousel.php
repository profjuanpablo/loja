<?php
	...
	if(empty($_POST['nome'])){
		$url = pg.'/adm/administrativo.php?link=74'; 
		echo "
	...
	if($salvar_dados_bd == 1){
		...
		$result_situacoes = "UPDATE situacoes_carrouses SET nome='$nome', cor='$cor', modified =  NOW() WHERE id = '$id'";
				...
				if(mysqli_affected_rows($conn) != 0){
					...
					echo "
						...
						<script type=\"text/javascript\">
							alert(\"Situação para Carrousel alterado com Sucesso.\");
						</script>
					";	
				}else{
					$url = pg.'/adm/administrativo.php?link=71'; 
					echo "
						<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=$url'>
						<script type=\"text/javascript\">
							alert(\"Situação para Carrousel não foi alterado com Sucesso.\");
						</script>
					";	
				}?>
			</body>
