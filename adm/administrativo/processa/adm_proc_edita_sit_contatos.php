<?php
	...
	if(empty($_POST['nome'])){
		$url = pg.'/adm/administrativo.php?link=78'; 
		echo "
	...
	if($salvar_dados_bd == 1){
		...
		$result_situacoes = "UPDATE situacoes_contatos SET nome='$nome', cor='$cor', modified =  NOW() WHERE id = '$id'";
				...
				if(mysqli_affected_rows($conn) != 0){
					...
					echo "
						...
						<script type=\"text/javascript\">
							alert(\"Situação para Mensagem de Contato alterado com Sucesso.\");
						</script>
					";	
				}else{
					$url = pg.'/adm/administrativo.php?link=75'; 
					echo "
						<META HTTP-EQUIV=REFRESH CONTENT = '0;URL=$url'>
						<script type=\"text/javascript\">
							alert(\"Situação para Mensagem de Contato não foi alterado com Sucesso.\");
						</script>
					";	
				}?>
			</body>
